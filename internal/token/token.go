// Package token implements struct and methods used to work with JWT tokens
package token
...
func NewEncryptedToken(id, email, username string, ttl int64, key string) (EncryptedJWT, error) {
	if ttl <= 0 {
		ttl = config.GlobalConfig.TTL.Access
	}
	...
	var err error
	t.Raw, err = t.SignedString([]byte(config.GlobalConfig.Secret))
	if err != nil {
