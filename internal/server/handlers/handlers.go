...
import (
	...
	"github.com/fpmi-hci-2023/project13b-auth/internal/cookie"
	"github.com/fpmi-hci-2023/project13b-auth/internal/model"
	"github.com/fpmi-hci-2023/project13b-auth/internal/response"
...
func NewAuthHandler(router fiber.Router, authService auth.AuthServiceClient) *AuthHandler {
	return &AuthHandler{
		Router:      router,
...
//	@Accept			json
//	@Produce		json
//	@Param			input				body		auth.RegisterRequest	true	"Register request"
...
func (h *AuthHandler) Registration(ctx *fiber.Ctx) error {
	...
	if err == nil { // if NO error, which means user is logged in
		...
		return ctx.Status(fiber.StatusConflict).JSON(response.Error{Error: ErrAlreadyLogged.Error()})
	}

	regRequest := &auth.RegisterRequest{}
	...
	// Access token cookie
	cookie.SetCookie(ctx, "accessToken", regResponse.AccessToken, config.GlobalConfig.TTL.Access)

	// Refresh token cookie
	cookie.SetCookie(ctx, "refreshToken", regResponse.RefreshToken, config.GlobalConfig.TTL.Refresh)
...
//	@Accept			json
//	@Produce		json
//	@Param			input			body		auth.LoginRequest	true	"username and password"
...
func (h *AuthHandler) Login(ctx *fiber.Ctx) error {
	...
	// Access token cookie
	cookie.SetCookie(ctx, "accessToken", loginResponse.AccessToken, config.GlobalConfig.TTL.Access)

	// Refresh token cookie
	cookie.SetCookie(ctx, "refreshToken", loginResponse.RefreshToken, config.GlobalConfig.TTL.Refresh)
...
func (h *AuthHandler) Validate(ctx *fiber.Ctx) error {
	...
	accessTokenTTL := config.GlobalConfig.TTL.Access
	refreshTokenTTL := config.GlobalConfig.TTL.Refresh

	switch validateResponse.TokenStatus {
	case auth.ValidateResponse_UPDATE:
...
func (h *AuthHandler) Info(ctx *fiber.Ctx) error {
	...
	infoRequest := &auth.ValidateRequest{
		AccessToken:  ctx.Cookies("accessToken"),
		RefreshToken: ctx.Cookies("refreshToken"),
	}
	...
	return ctx.Status(fiber.StatusOK).JSON(auth.InfoResponse{
		Id:        infoResponse.Id,
